...
const IntradayViewer: React.FC = () => {
  const [dailyChartData, setDailyChartData] = useState<ChartData | null>(null);

  const [symbolStatus, setSymbolStatus] = useState(useAppSelector(selectSymbol) || 'TSLA');
  const [currentPrice, setCurrentPrice] = useState<number | null>(null);
  // const [currentPriceStatic, setCurrentPriceStatic] = useState<number | null>(null);
  ...
  const fetchDataDaily = async (symbol: string) => {
    try {
      ...
      if (data && typeof data === 'string') {
        ...
        setErrorDaily(true);
        // dispatch(setErrorDailyChart(true));
      }
      setErrorDaily(true);
      // dispatch(setErrorDailyChart(false));
    } catch (error) {
  ...
  const fetchDataWeekly = async (symbol: string) => {
    try {
      ...
      if (data && typeof data === 'string') {
        ...
        dispatch(setChartDataWeekly(newChartData));
        // toast.success(`Showing data for ${symbol}`);
      } else {
        console.error('Error fetching data: Unexpected data format');
        // toast.error('Error fetching data');
  ...
  const fetchCurrentPrice = async () => {
    try {
      ...
      if (data && data.current_price) {
        ...
        if (!isMarketOpen()) { // Check if market is open and set currentPrice with a static symbol value
  ...
  // Function to parse CSV data and extract timestamps and close prices
  ...
  //useeffect to fetchcurrentprice every 4.5 seconds
  useEffect(() => {
  ...
  // useeffect to make sure price percentage is updated with daily data even if the active tab is weekly
  useEffect(() => {
    if (activeTab === 'weekly') {
      setTimeout(() => {
  ...
  const checkMarketStatus = () => {
    ...
    // Check if it's a weekend (Saturday or Sunday)
    if (dayOfWeek === 0 || dayOfWeek === 6) {
      setMarketStatus('Market is currently closed. It will reopen on Monday.');
      return;
    }
    ...
    if (
      ...
      const totalMinutesUntilClose = 16 * 60 - (currentHour * 60 + currentMinute);
      const hoursUntilClose = Math.floor(totalMinutesUntilClose / 60);
      const minutesUntilClose = totalMinutesUntilClose % 60;

      let timeUntilCloseMessage = '';

      if (hoursUntilClose > 0) {
        timeUntilCloseMessage += `${hoursUntilClose} ${hoursUntilClose > 1 ? 'hours' : 'hour'}`;
      }

      if (minutesUntilClose > 0) {
        if (timeUntilCloseMessage !== '') {
          timeUntilCloseMessage += ' and ';
        }
        timeUntilCloseMessage += `${minutesUntilClose} ${minutesUntilClose > 1 ? 'minutes' : 'minute'}`;
      }

      if (timeUntilCloseMessage === '') {
        timeUntilCloseMessage = 'less than a minute';
      }


      setMarketStatus(`The market is currently open. It will close in ${timeUntilCloseMessage}.`);
      // setMarketStatus('Market is currently open for trading.'); // backup
    } else {
      // Calculate the time until the market opens
      const nextOpenTime = new Date(currentDateTime);
      if (currentHour > 16 || (currentHour === 16 && currentMinute > 0)) {
        nextOpenTime.setDate(nextOpenTime.getDate() + 1); // Next trading day
      }
      nextOpenTime.setHours(9, 30, 0); // Market opens at 9:30 AM on the next trading day

      const timeUntilOpen = nextOpenTime.getTime() - currentDateTime.getTime();
      const hoursUntilOpen = Math.floor(timeUntilOpen / (1000 * 60 * 60));
      const minutesUntilOpen = Math.ceil((timeUntilOpen % (1000 * 60 * 60)) / (1000 * 60));

      let timeUntilOpenMessage = '';

      if (hoursUntilOpen > 0) {
        timeUntilOpenMessage += `${hoursUntilOpen} ${hoursUntilOpen > 1 ? 'hours' : 'hour'}`;
      }

      if (minutesUntilOpen > 0) {
        if (timeUntilOpenMessage !== '') {
          timeUntilOpenMessage += ' and ';
        }
        timeUntilOpenMessage += `${minutesUntilOpen} ${minutesUntilOpen > 1 ? 'minutes' : 'minute'}`;
      }

      if (timeUntilOpenMessage === '') {
        timeUntilOpenMessage = 'less than a minute';
      }

      setMarketStatus(`The market is currently closed. It will reopen in ${timeUntilOpenMessage}.`);
    }
  };

  // Use setInterval to check market status periodically
  useEffect(() => {
  ...
  return (
    <div
      className="bg-img">
      <h1 style={{ fontWeight: 'bold', textAlign: 'center' }} className="animate__animated animate__flipInX">Index Tracker</h1>
      <br />
      ...
      <Autocomplete
        ...
        sx={{
          ...
          justifyContent: 'center', margin: 'auto', paddingBottom: '15px', '& .MuiInputBase-input': { fontSize: '14px', color: 'white' }
        }}
        getOptionLabel={(option) => `${option.Symbol} (${option.Name})`}
        renderInput={(params) =>
          <TextField
            {...params}
            label="NASDAQ Symbol Search"
            InputLabelProps={{
              style: { color: 'white' },
            }}
          />}
      ...
      <Button variant="contained" sx={{ fontSize: '14px', textTransform: 'none' }} onClick={sentimentAI} endIcon={<AutoAwesomeIcon />}>AI Sentiment Tool</Button>
      ...
      <h4 style={{ color: isMarketOpen() ? 'green' : 'grey', margin: '0 auto', marginBottom: '10px', width: '90%' }}>{marketStatus}</h4>
      <br />
      {currentPrice === null || !loading ? (
        ...
        <h3 style={{ margin: '0 auto', marginBottom: '10px', width: '90%' }} className='animate__animated animate__fadeInUp'>
          Current Price for {symbolStatus} ({stockSymbols.find(stock => stock.Symbol === symbolStatus)?.Name}): <span style={{ color: priceChangeColor, fontWeight: 'bold' }}>${formattedPrice}</span>
      ...
      {symbolStatus === null || loading ?
        <Button variant="contained" sx={{ marginBottom: '15px', marginTop: '15px', fontSize: '14px', textTransform: 'none' }} onClick={addToWatchlist} endIcon={<AddCircleOutlineIcon />}>Add to Watchlist</Button> :
        <Button variant="contained" sx={{ marginBottom: '15px', marginTop: '15px', fontSize: '14px', textTransform: 'none' }} disabled endIcon={<AddCircleOutlineIcon />}>Add to Watchlist</Button>}
      ...
      {finalChartData !== null ? (
        <div style={{ margin: '0 auto', width: '92%', border: '2px solid white', borderRadius: '10px', padding: '5px' }}>
          ...
          {null && <ChartComponent activeTab={activeTab} />} {/* backup alternative Chart component for future expansions */}
        </div>
      ) : (
        <>
      ...
      <br />
      {showVolume && (<div>
        <h4 style={{ textAlign: 'center' }}>Volume (24H)</h4>
